...
public class MonteCarloTest {
    public static void piDemo() {
        ...
        BiFunction<Long, Supplier<Integer>, Double> monteCarlo = (n,
                                                                  experiment) ->
                Mathx.sum(limit(generate(experiment), n)) / n;
        ...
        Function<Supplier<Integer>, Iterator<Double>> monteCarloIterator = experiment -> zip((x, y) -> (x / y),
                iterate(0D, x -> x + experiment.get()), iterate(1D, x -> x + 1));
    ...
    private static void potionTestWithInfiniteIterators() {
        ...
        Iterator<Quality> herbQualities = zip((x, y) -> x == 1 ? Quality.BEST : Quality.values()[y]
